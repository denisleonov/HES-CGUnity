    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                fixed4 cx = tex2D(_XAlbedo, i.wPos.yz * _Scale);
                fixed4 cy = tex2D(_YAlbedo, i.wPos.xz * _Scale);
                fixed4 cz = tex2D(_ZAlbedo, i.wPos.xy * _Scale);
                
                float4 albedo = pow(i.normal.x, 2) * cx + pow(i.normal.y, 2) * cy + pow(i.normal.z, 2) * cz;
